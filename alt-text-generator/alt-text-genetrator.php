<?php
/**
 * Plugin Name: Copy Title to Alt Text
...
class Copy_Title_To_Alt {
    ...
    public function enqueue_scripts($hook) {
        if ($hook !== 'tools_page_copy-title-to-alt') {
            return;
        }
        
        wp_enqueue_script('copy-title-to-alt-js', plugin_dir_url(__FILE__) . 'script.js', array('jquery'), '1.1', true);
        wp_localize_script('copy-title-to-alt-js', 'copyTitleAjax', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('copy_title_batch_nonce')
        ));
    }
    ...
    public function admin_page() {
        ...
        jQuery(document).ready(function($) {
            ...
            $('#start-batch').on('click', function() {
                ...
                $.post(copyTitleAjax.ajax_url, {
                    action: 'get_image_count',
                    nonce: copyTitleAjax.nonce
                }, function(response) {
                    if (response.success) {
