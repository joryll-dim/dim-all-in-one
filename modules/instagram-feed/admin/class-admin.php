...
class DIM_Instagram_Admin {
    ...
    public function enqueue_styles($hook) {
        if ($hook != 'settings_page_dim-instagram-settings') {
            return;
        }
    ...
    public function settings_init() {
        if ($this->settings === null) {
            if (!class_exists('DIM_Instagram_Settings')) {
                require_once plugin_dir_path(__FILE__) . 'class-settings.php';
            }
            $this->settings = new DIM_Instagram_Settings($this->plugin_name, $this->version);
        }
